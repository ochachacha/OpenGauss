...
The evaluate flow:
    ...
    2. evaluate()  -- Iterates over all runs sequentially through:
        a. rollout_and_score_eval()  -- Per-run agent loop
            ...
            - Runs GaussAgentLoop with terminal tool only
...
Requires: pip install gauss-agent[yc-bench]
"""
...
class YCBenchEvalConfig(GaussAgentEnvConfig):
    """
    Configuration for the YC-Bench evaluation environment.

    Extends GaussAgentEnvConfig with YC-Bench-specific settings for
...
class YCBenchEvalEnv(GaussAgentBaseEnv):
    """
    ...
    a competing built-in agent loop). The GaussAgentLoop then drives the
    ...
    async def rollout_and_score_eval(self, eval_item: Dict[str, Any]) -> Dict:
        ...
        3. Runs GaussAgentLoop with terminal tool
        ...
        try:
            ...
            # LiteLLM), which would compete with our GaussAgentLoop.
