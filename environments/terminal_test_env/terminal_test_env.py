...
Training tasks (3):
    1. Create ~/greeting.txt with "Hello from Gauss Agent"
...
TRAIN_TASKS = [
    {
        "prompt": "Create a file at ~/greeting.txt containing exactly the text: Hello from Gauss Agent",
        "verify_path": "~/greeting.txt",
        "expected_content": "Hello from Gauss Agent",
    },
    {
...
class TerminalTestEnv(GaussAgentBaseEnv):
    ...
    def config_init(cls) -> Tuple[TerminalTestEnvConfig, List[APIServerConfig]]:
        ...
        Claude for inference. API keys loaded from ~/gauss-agent/.env.
        """
