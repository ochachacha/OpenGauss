"""
GaussSweEnv -- SWE-Bench Style Environment with Modal Sandboxes
...
Usage:
    ...
    vllm serve YourModel --tool-parser gauss
    run-api
    python environments/gauss_swe_env.py serve \\
...
class GaussSweEnv(GaussAgentBaseEnv):
    ...
    name = "gauss-swe"
    env_config_cls = GaussSweEnvConfig

    @classmethod
    def config_init(cls) -> Tuple[GaussSweEnvConfig, List[APIServerConfig]]:
        """
        ...
        env_config = GaussSweEnvConfig(
            ...
            # Atropos settings
            group_size=4,
            tokenizer_name="NousResearch/DeepGauss-3-Llama-3-3B-Preview",
            tool_call_parser="gauss",
            steps_per_eval=50,
            total_steps=500,
            use_wandb=True,
            wandb_name="gauss-swe",
        )

        server_configs = [
            APIServerConfig(
                base_url="http://localhost:8000/v1",
                model_name="NousResearch/DeepGauss-3-Llama-3-3B-Preview",
