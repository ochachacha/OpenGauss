...
class TestSendViaRestApi:
    ...
    async def test_send_success(self):
        ...
        assert call_args[1]["json"]["title"] == "Gauss Agent"
...
class TestToolsetIntegration:
    def test_homeassistant_toolset_is_not_exposed(self):
        from toolsets import resolve_toolset

        tools = resolve_toolset("homeassistant")
        assert tools == []

    def test_gateway_toolset_excludes_ha_tools(self):
        from toolsets import resolve_toolset

        gateway_tools = resolve_toolset("gauss-gateway")
        for tool in ("ha_list_entities", "ha_get_state", "ha_call_service", "ha_list_services"):
            assert tool not in gateway_tools

    def test_gauss_core_tools_excludes_ha(self):
        from toolsets import _GAUSS_CORE_TOOLS

        for tool in ("ha_list_entities", "ha_get_state", "ha_call_service", "ha_list_services"):
            assert tool not in _GAUSS_CORE_TOOLS
