...
def adapter():
    ...
    adapter._client = SimpleNamespace(
        ...
        user=SimpleNamespace(id=99999, name="GaussBot"),
    )
    return adapter
...
@pytest.mark.asyncio
async def test_registers_managed_workflow_slashes_and_drops_reload_mcp(adapter):
    ...
    assert "reload-mcp" not in adapter._client.tree.commands

    interaction = SimpleNamespace()
    await adapter._client.tree.commands["autoformalize"](interaction, goal="Formalize theorem 3.1")

    adapter._run_simple_slash.assert_awaited_once_with(
        interaction,
        "/autoformalize Formalize theorem 3.1",
    )
...
async def test_handle_thread_create_slash_dispatches_session_when_message_provided(adapter):
    ...
    await adapter._handle_thread_create_slash(interaction, "Planning", "Hello Gauss", 1440)

    adapter._dispatch_thread_session.assert_awaited_once_with(
        interaction, "555", "Planning", "Hello Gauss",
    )
...
def test_discord_auto_thread_config_bridge(monkeypatch, tmp_path):
    ...
    gauss_dir = tmp_path / ".gauss"
    gauss_dir.mkdir()
    config_path = gauss_dir / "config.yaml"
    config_path.write_text(yaml.dump({
        "discord": {"auto_thread": True},
    }))

    monkeypatch.delenv("DISCORD_AUTO_THREAD", raising=False)
    monkeypatch.setenv("GAUSS_HOME", str(gauss_dir))
