...
def _ensure_telegram_mock():
    ...
    telegram_mod.BotCommand = lambda command, description: SimpleNamespace(
        command=command,
...
@pytest.mark.asyncio
async def test_connect_registers_managed_workflow_bot_commands(monkeypatch):
    adapter = TelegramAdapter(PlatformConfig(enabled=True, token="secret-token"))
    ...
    assert "reload_mcp" not in names
