...
def _redirect_cache(tmp_path, monkeypatch):
    """Point document cache to tmp_path so tests don't touch ~/.gauss."""
    monkeypatch.setattr(
...
class TestAppMentionHandler:
    ...
    def test_app_mention_registered_on_connect(self):
        ...
        assert "/gauss" in registered_commands
...
class TestSlashCommands:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("handoff Formalize theorem 1.1", "/handoff Formalize theorem 1.1"),
            ("plan Formalize theorem 1.1", "/plan Formalize theorem 1.1"),
            ("skills browse lean", "/skills browse lean"),
            ("reload-mcp", "/reload-mcp"),
        ],
    )
    async def test_gateway_commands_map_through_slash(self, adapter, text, expected):
        command = {"text": text, "user_id": "U1", "channel_id": "C1"}
        await adapter._handle_slash_command(command)
        msg = adapter.handle_message.call_args[0][0]
