"""Tests that provider selection via `gauss model` always persists correctly.
...
def config_home(tmp_path, monkeypatch):
    """Isolated GAUSS_HOME with a minimal string-format config."""
    home = tmp_path / "gauss"
    home.mkdir()
    ...
    env_file = home / ".env"
    env_file.write_text("")
    monkeypatch.setenv("GAUSS_HOME", str(home))
    # Clear env vars that could interfere
    monkeypatch.delenv("GAUSS_MODEL", raising=False)
    monkeypatch.delenv("LLM_MODEL", raising=False)
    monkeypatch.delenv("GAUSS_INFERENCE_PROVIDER", raising=False)
...
class TestSaveModelChoiceAlwaysDict:
    def test_string_model_becomes_dict(self, config_home):
        ...
        from gauss_cli.auth import _save_model_choice
...
class TestProviderPersistsAfterModelSave:
    def test_api_key_provider_saved_when_model_was_string(self, config_home, monkeypatch):
        ...
        from gauss_cli.main import _model_flow_api_key_provider
        ...
        with patch("gauss_cli.auth._prompt_model_selection", return_value="kimi-k2.5"), \
             patch("gauss_cli.auth.deactivate_provider"), \
