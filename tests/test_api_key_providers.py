...
class TestHasAnyProviderConfigured:
    ...
    def test_glm_key_counts(self, monkeypatch, tmp_path):
        from gauss_cli import config as config_module
        monkeypatch.setenv("GLM_API_KEY", "test-key")
        gauss_home = tmp_path / ".gauss"
        gauss_home.mkdir()
        monkeypatch.setattr(config_module, "get_env_path", lambda: gauss_home / ".env")
        monkeypatch.setattr(config_module, "get_gauss_home", lambda: gauss_home)
        from gauss_cli.main import _has_any_provider_configured
...
class TestKimiMoonshotModelListIsolation:
    ...
    def test_moonshot_list_excludes_coding_plan_only_models(self):
        from gauss_cli.main import _PROVIDER_MODELS
