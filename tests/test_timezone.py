"""
Tests for timezone support (gauss_time module + integration points).
...
# =========================================================================
# gauss_time.now() — core helper
# =========================================================================

class TestGaussTimeNow:
    ...
    def teardown_method(self):
        gauss_time.reset_cache()
        os.environ.pop("GAUSS_TIMEZONE", None)

    def test_valid_timezone_applies(self):
        """With a valid IANA timezone, now() returns time in that zone."""
        os.environ["GAUSS_TIMEZONE"] = "Asia/Kolkata"
        result = gauss_time.now()
    ...
    def test_us_eastern(self):
        ...
        os.environ["GAUSS_TIMEZONE"] = "America/New_York"
        result = gauss_time.now()
        assert result.tzinfo is not None
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["GAUSS_TIMEZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="gauss_time"):
            result = gauss_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["GAUSS_TIMEZONE"] = "Europe/London"
        tz = gauss_time.get_timezone()
    ...
    def test_returns_none_for_invalid(self):
        os.environ["GAUSS_TIMEZONE"] = "Not/A/Timezone"
        tz = gauss_time.get_timezone()
        assert tz is None

    def test_get_timezone_name(self):
        os.environ["GAUSS_TIMEZONE"] = "Asia/Tokyo"
        assert gauss_time.get_timezone_name() == "Asia/Tokyo"
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When GAUSS_TIMEZONE is set, child process sees TZ env var."""
        import json as _json
        os.environ["GAUSS_TIMEZONE"] = "Asia/Kolkata"
    ...
    def test_tz_not_injected_when_empty(self):
        """When GAUSS_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("GAUSS_TIMEZONE", None)
    ...
    def test_gauss_timezone_not_leaked_to_child(self):
        """GAUSS_TIMEZONE itself must NOT appear in child env (only TZ)."""
        import json as _json
        os.environ["GAUSS_TIMEZONE"] = "Asia/Kolkata"

        with patch("model_tools.handle_function_call", side_effect=self._mock_handle):
            result = _json.loads(self._execute_code(
                code='import os; print(os.environ.get("GAUSS_TIMEZONE", "NOT_SET"))',
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=gauss_tz) which shifted
        absolute time when system-local tz != Gauss tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_gauss_tz(self):
        """Already-aware datetimes should be normalized to Gauss tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in Gauss tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of Gauss tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), Gauss is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Gauss tz
        ...
        # Use a Gauss timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _gauss_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["GAUSS_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        gauss_time.reset_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Gauss timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["GAUSS_TIMEZONE"] = "US/Eastern"
        gauss_time.reset_cache()
