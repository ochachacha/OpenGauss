...
class TestCreateSession:
    ...
    def test_create_session_normalizes_relative_cwd(self, manager):
        state = manager.create_session(cwd=".")
        assert Path(state.cwd).is_absolute()
...
class TestListAndCleanup:
    ...
    def test_list_sessions_filters_by_cwd(self, manager):
        manager.create_session(cwd="/a")
        kept = manager.create_session(cwd="/b")
        listing = manager.list_sessions(cwd="/b")
        assert [item["session_id"] for item in listing] == [kept.session_id]
