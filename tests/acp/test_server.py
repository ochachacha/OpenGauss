"""Tests for acp_adapter.server — GaussACPAgent ACP server."""
...
from acp_adapter.server import GaussACPAgent, GAUSS_VERSION
...
def agent(mock_manager):
    """GaussACPAgent backed by a mock session manager."""
    return GaussACPAgent(session_manager=mock_manager)
...
class TestInitialize:
    ...
    async def test_initialize_returns_agent_info(self, agent):
        ...
        assert resp.agent_info.name == "gauss-agent"
        assert resp.agent_info.version == GAUSS_VERSION
...
class TestAuthenticate:
    ...
    @pytest.mark.asyncio
    async def test_authenticate_rejects_wrong_method_id(self, agent, monkeypatch):
        monkeypatch.setattr("acp_adapter.server.detect_provider", lambda: "openrouter")
...
class TestSessionOps:
    ...
    async def test_resume_session_creates_new_if_missing(self, agent):
        ...
        assert isinstance(resume_resp, ResumeSessionResponse)
        assert agent.session_manager.get_session("nonexistent") is not None
...
class TestListAndFork:
    ...
    async def test_list_sessions(self, agent):
        ...
        assert all(os.path.isabs(session.cwd) for session in resp.sessions)

    @pytest.mark.asyncio
    async def test_list_sessions_filters_by_absolute_cwd(self, agent):
        cwd_a = os.path.abspath("/a")
        cwd_b = os.path.abspath("/b")
        await agent.new_session(cwd=cwd_a)
        await agent.new_session(cwd=cwd_b)
        resp = await agent.list_sessions(cwd=cwd_a)
        assert isinstance(resp, ListSessionsResponse)
        assert [session.cwd for session in resp.sessions] == [cwd_a]
    ...
    @pytest.mark.asyncio
    async def test_fork_session_missing_source_raises_invalid_params(self, agent):
        with pytest.raises(RequestError) as exc:
            await agent.fork_session(cwd="/forked", session_id="missing")
...
class TestSlashCommands:
    ...
    def test_model_switch_passes_provider_override(self, agent, mock_manager, monkeypatch):
        state = self._make_state(mock_manager)
        replacement_agent = MagicMock(name="ReplacementAgent")
        replacement_agent.provider = "openai"
        replacement_agent.model = "gpt-4.1"

        make_agent = MagicMock(return_value=replacement_agent)
        monkeypatch.setattr(agent.session_manager, "_make_agent", make_agent)
        monkeypatch.setattr("gauss_cli.models.parse_model_input", lambda model, current: ("openai", "gpt-4.1"))
        monkeypatch.setattr("gauss_cli.models.detect_provider_for_model", lambda model, current: None)

        result = agent._handle_slash_command("/model openai/gpt-4.1", state)
    ...
    def test_version(self, agent, mock_manager):
        ...
        assert GAUSS_VERSION in result
    ...
    @pytest.mark.asyncio
    async def test_set_session_model_uses_detected_provider(self, agent, mock_manager, monkeypatch):
        new_resp = await agent.new_session(cwd="/tmp")
        state = agent.session_manager.get_session(new_resp.session_id)
        state.agent.provider = "openrouter"
        ...
        await agent.set_session_model(model_id="openai/gpt-4.1", session_id=new_resp.session_id)
