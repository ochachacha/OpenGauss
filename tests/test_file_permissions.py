...
class TestConfigFilePermissions(unittest.TestCase):
    ...
    def test_save_config_sets_0600(self):
        ...
        with patch("gauss_cli.config.get_config_path", return_value=config_path), \
             patch("gauss_cli.config.ensure_gauss_home"):
            from gauss_cli.config import save_config
    ...
    def test_save_env_value_sets_0600(self):
        ...
        with patch("gauss_cli.config.get_env_path", return_value=env_path), \
             patch("gauss_cli.config.ensure_gauss_home"):
    ...
    def test_ensure_gauss_home_sets_0700(self):
        home = Path(self.tmpdir) / ".gauss"
        with patch("gauss_cli.config.get_gauss_home", return_value=home):
