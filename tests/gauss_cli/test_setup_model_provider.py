...
from gauss_cli.config import load_config, save_config, save_env_value
from gauss_cli.setup import _print_setup_summary, setup_model_provider
...
def test_setup_keep_current_custom_from_config_does_not_fall_through(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("gauss_cli.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("gauss_cli.setup.prompt", lambda *args, **kwargs: "")
    monkeypatch.setattr("gauss_cli.setup.prompt_yes_no", lambda *args, **kwargs: False)
    monkeypatch.setattr("gauss_cli.auth.get_active_provider", lambda: None)
...
def test_setup_custom_endpoint_saves_working_v1_base_url(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("gauss_cli.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("gauss_cli.setup.prompt", fake_prompt)
    monkeypatch.setattr("gauss_cli.setup.prompt_yes_no", lambda *args, **kwargs: False)
    monkeypatch.setattr("gauss_cli.auth.get_active_provider", lambda: None)
    monkeypatch.setattr("gauss_cli.auth.detect_external_credentials", lambda: [])
...
def test_setup_keep_current_config_provider_uses_provider_specific_model_menu(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("gauss_cli.auth.get_active_provider", lambda: None)
    monkeypatch.setattr("gauss_cli.auth.detect_external_credentials", lambda: [])
    monkeypatch.setattr("gauss_cli.models.provider_model_ids", lambda provider: [])
    monkeypatch.setattr("agent.auxiliary_client.get_available_vision_backends", lambda: [])
...
def test_setup_keep_current_anthropic_can_configure_openai_vision_default(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("gauss_cli.setup.prompt_choice", lambda *args, **kwargs: next(picks))
    monkeypatch.setattr(
...
def test_setup_summary_marks_codex_auth_as_vision_available(tmp_path, monkeypatch, capsys):
    ...
    assert "missing run 'gauss setup' to configure" not in output
