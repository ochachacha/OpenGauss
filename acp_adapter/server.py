"""ACP agent server — exposes Gauss Agent via the Agent Client Protocol."""
...
try:
    from gauss_cli import __version__ as GAUSS_VERSION
except Exception:
    GAUSS_VERSION = "0.0.0"
...
class GaussACPAgent(acp.Agent):
    """ACP Agent implementation wrapping Gauss AIAgent."""
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="gauss-agent", version=GAUSS_VERSION),
    ...
    async def resume_session(
        ...
        **kwargs: Any,
        ...
        state = self.session_manager.update_cwd(session_id, cwd)
        if state is None:
            logger.warning("resume_session: session %s not found, creating fresh state under requested id", session_id)
    ...
    async def fork_session(
        ...
        **kwargs: Any,
        ...
        if state is None:
            raise RequestError.invalid_params({"session_id": session_id, "message": "Session not found"})
    ...
    async def list_sessions(
        ...
        **kwargs: Any,
        ...
        infos = self.session_manager.list_sessions(cwd=requested_cwd)
        sessions = [
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Gauss on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
    ...
    def _cmd_model(self, args: str, state: SessionState) -> str:
        ...
        try:
            from gauss_cli.models import parse_model_input, detect_provider_for_model
    ...
    def _cmd_tools(self, args: str, state: SessionState) -> str:
        try:
            ...
            toolsets = getattr(state.agent, "enabled_toolsets", None) or ["gauss-acp"]
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Gauss Agent v{GAUSS_VERSION}"
    ...
    async def set_session_model(
        ...
        if state:
            ...
            try:
                ...
                logger.debug("Provider detection failed during ACP model switch", exc_info=True)
            ...
            logger.info("Session %s: model switched to %s", session_id, new_model)
        return None
