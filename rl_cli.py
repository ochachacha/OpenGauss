...
if tinker_atropos_dir.exists():
    ...
    os.environ['GAUSS_QUIET'] = '1'  # Disable temp subdirectory creation
    print(f"📂 Terminal working directory: {tinker_atropos_dir}")
else:
    # Fall back to gauss-agent directory if submodule not found
...
def main(
    ...
    Args:
        ...
        model: Model to use for the agent (reads from ~/.gauss/config.yaml if not provided)
    ...
    """
    # Load config from ~/.gauss/config.yaml
    config = load_gauss_config()
