...
Checklist for integrating a new messaging platform into the Gauss gateway.
...
And add it to the `gauss-gateway` composite:

```python
"gauss-gateway": {
    "includes": [..., "gauss-your-platform"]
}
```
...
## 12. Status Display (`gauss_cli/status.py`)
...
## 13. Gateway Setup Wizard (`gauss_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ gauss_cli/ toolsets.py \
  --include="*.py" -l | sort -u
