...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="ask", description="Ask Gauss a question")
        @discord.app_commands.describe(question="Your question for Gauss")
        ...
        @tree.command(name="reset", description="Reset your Gauss session")
        ...
        @tree.command(name="status", description="Show Gauss session status")
        ...
        @tree.command(name="stop", description="Stop the running Gauss agent")
        ...
        @tree.command(name="prove", description="Show how to launch guided Lean proving from the CLI")
        @discord.app_commands.describe(scope="Optional file or theorem to pass through.")
        async def slash_prove(interaction: discord.Interaction, scope: str = ""):
            await self._run_simple_slash(interaction, f"/prove {scope}".strip())

        @tree.command(name="draft", description="Show how to launch Lean drafting from the CLI")
        @discord.app_commands.describe(claim="Optional informal claim to pass through.")
        ...
        async def slash_autoprove(interaction: discord.Interaction, scope: str = ""):
            await self._run_simple_slash(interaction, f"/autoprove {scope}".strip())

        @tree.command(name="formalize", description="Show how to launch interactive Lean formalization from the CLI")
        @discord.app_commands.describe(claim="Optional theorem or source-backed request to pass through.")
        async def slash_formalize(interaction: discord.Interaction, claim: str = ""):
            await self._run_simple_slash(interaction, f"/formalize {claim}".strip())

        @tree.command(name="autoformalize", description="Show how to launch autonomous Lean formalization from the CLI")
        @discord.app_commands.describe(goal="Optional Lean goal or theorem to pass through.")
        async def slash_autoformalize(interaction: discord.Interaction, goal: str = ""):
            await self._run_simple_slash(interaction, f"/autoformalize {goal}".strip())
        ...
        @tree.command(name="update", description="Update Gauss Agent to the latest version")
        async def slash_update(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/update", "Update initiated~")

        @tree.command(name="thread", description="Create a new thread and start a Gauss session in it")
        @discord.app_commands.describe(
            name="Thread name",
            message="Optional first message to send to Gauss in the thread",
    ...
    async def _handle_thread_create_slash(
        ...
        # If a message was provided, kick off a new Gauss session in the thread
    ...
    async def _create_thread(
        ...
        *,
        ...
        try:
            ...
            try:
                seed_content = starter_message or f"\U0001f9f5 Thread created by Gauss: **{name}**"
    ...
    async def _auto_create_thread(self, message: 'DiscordMessage') -> Optional[Any]:
        ...
        thread_name = content[:80] if content else "Gauss"
        if len(content) > 80:
