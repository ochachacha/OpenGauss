...
# Resolve Gauss home directory (respects GAUSS_HOME override)
_gauss_home = Path(os.getenv("GAUSS_HOME", Path.home() / ".gauss"))

# Load environment variables from ~/.gauss/.env first.
# User-managed env files should override stale shell exports on restart.
from dotenv import load_dotenv  # backward-compat for tests that monkeypatch this symbol
from gauss_cli.env_loader import load_gauss_dotenv
_env_path = _gauss_home / '.env'
load_gauss_dotenv(gauss_home=_gauss_home, project_env=Path(__file__).resolve().parents[1] / '.env')

# Bridge config.yaml values into the environment so os.getenv() picks them up.
# config.yaml is authoritative for terminal settings — overrides .env.
_config_path = _gauss_home / 'config.yaml'
    ...
    try:
        ...
        if _agent_cfg and isinstance(_agent_cfg, dict):
            if "max_turns" in _agent_cfg:
                os.environ["GAUSS_MAX_ITERATIONS"] = str(_agent_cfg["max_turns"])
        # Timezone: bridge config.yaml → GAUSS_TIMEZONE env var.
        # GAUSS_TIMEZONE from .env takes precedence (already in os.environ).
        _tz_cfg = _cfg.get("timezone", "")
        if _tz_cfg and isinstance(_tz_cfg, str) and "GAUSS_TIMEZONE" not in os.environ:
            os.environ["GAUSS_TIMEZONE"] = _tz_cfg.strip()
        # Security settings
        _security_cfg = _cfg.get("security", {})
        if isinstance(_security_cfg, dict):
            _redact = _security_cfg.get("redact_secrets")
            if _redact is not None:
                os.environ["GAUSS_REDACT_SECRETS"] = str(_redact).lower()
    except Exception:
        pass  # Non-fatal; gateway can still run with .env values

# Gateway runs in quiet mode - suppress debug output and use cwd directly (no temp dirs)
os.environ["GAUSS_QUIET"] = "1"

# Enable interactive exec approval for dangerous commands on messaging platforms
os.environ["GAUSS_EXEC_ASK"] = "1"
...
def _resolve_runtime_agent_kwargs() -> dict:
    ...
    try:
        runtime = resolve_runtime_provider(
            requested=os.getenv("GAUSS_INFERENCE_PROVIDER"),
        )
...
def _resolve_gateway_model() -> str:
    ...
    """
    model = os.getenv("GAUSS_MODEL") or os.getenv("LLM_MODEL") or "anthropic/claude-opus-4.6"
    try:
        import yaml as _y
        _cfg_path = _gauss_home / "config.yaml"
        if _cfg_path.exists():
...
def _resolve_gauss_bin() -> Optional[list[str]]:
    """Resolve the Gauss update command as argv parts.

    Tries in order:
    1. ``shutil.which("gauss")`` — standard PATH lookup
    2. ``sys.executable -m gauss_cli.main`` — fallback when Gauss is running
       from a venv/module invocation and the ``gauss`` shim is not on PATH
    ...
    try:
        ...
        if importlib.util.find_spec("gauss_cli") is not None:
            return [sys.executable, "-m", "gauss_cli.main"]
    except Exception:
        pass
...
class GatewayRunner:
    ...
    _VOICE_MODE_PATH = _gauss_home / "gateway_voice_mode.json"
    ...
    def _load_prefill_messages() -> List[Dict[str, Any]]:
        ...
        Checks GAUSS_PREFILL_MESSAGES_FILE env var first, then falls back to
        the prefill_messages_file key in ~/.gauss/config.yaml.
        Relative paths are resolved from ~/.gauss/.
        """
        import json as _json
        file_path = os.getenv("GAUSS_PREFILL_MESSAGES_FILE", "")
        if not file_path:
            try:
                import yaml as _y
                cfg_path = _gauss_home / "config.yaml"
                if cfg_path.exists():
    ...
    def _load_ephemeral_system_prompt() -> str:
        ...
        Checks GAUSS_EPHEMERAL_SYSTEM_PROMPT env var first, then falls back to
        agent.system_prompt in ~/.gauss/config.yaml.
        """
        prompt = os.getenv("GAUSS_EPHEMERAL_SYSTEM_PROMPT", "")
        if prompt:
            return prompt
        try:
            import yaml as _y
    ...
    def _load_reasoning_config() -> dict | None:
        ...
        GAUSS_REASONING_EFFORT as a fallback. Valid: "xhigh", "high",
        ...
        try:
            ...
            pass
        if not effort:
            effort = os.getenv("GAUSS_REASONING_EFFORT", "")
        if not effort:
            return None
    ...
    def _load_background_notifications_mode() -> str:
        ...
        """
        mode = os.getenv("GAUSS_BACKGROUND_NOTIFICATIONS", "")
        if not mode:
            try:
                import yaml as _y
                cfg_path = _gauss_home / "config.yaml"
    ...
    async def start(self) -> bool:
        ...
        """
        logger.info("Starting Gauss Gateway...")
        ...
        if not _any_allowlist and not _allow_all:
            logger.warning(
                ...
                "Set GATEWAY_ALLOW_ALL_USERS=true in ~/.gauss/.env to allow open access, "
        ...
        if not notified and any(
            path.exists()
            for path in (
                _gauss_home / ".update_pending.json",
                _gauss_home / ".update_pending.claimed.json",
            )
        ):
    ...
    def _create_adapter(
        ...
        if platform == Platform.TELEGRAM:
            ...
            if not check_slack_requirements():
                logger.warning("Slack: slack-bolt not installed. Run: pip install 'gauss-agent[slack]'")
                return None
    ...
    async def _handle_message(self, event: MessageEvent) -> Optional[str]:
                ...
                if code:
                    ...
                    if adapter:
                        await adapter.send(
                            ...
                            f"`gauss pairing approve {platform_name} {code}`"
                        )
                else:
                    adapter = self.adapters.get(source.platform)
                    if adapter:
        ...
        if command == "reload-mcp":
            return (
                "User-managed MCP reload is not part of the default Gauss surface. "
                "Managed Lean workflows use an isolated Lean MCP config automatically."
            )
        ...
        if history and len(history) >= 4:
            ...
            try:
                _hyg_cfg_path = _gauss_home / "config.yaml"
            ...
            if not os.getenv(env_key):
                ...
                if adapter:
                    await adapter.send(
                        ...
                        f"A home channel is where Gauss delivers cron job results "
    ...
    async def _handle_help_command(self, event: MessageEvent) -> str:
        ...
        lines = [
            ...
            "`/update` — Update Gauss to the latest version",
    ...
    async def _handle_model_command(self, event: MessageEvent) -> str:
        ...
        args = event.get_command_args().strip()
        config_path = _gauss_home / 'config.yaml'

        # Resolve current model and provider from config
        current = os.getenv("GAUSS_MODEL") or "anthropic/claude-opus-4.6"
        ...
        os.environ["GAUSS_MODEL"] = new_model
        if provider_changed:
            os.environ["GAUSS_INFERENCE_PROVIDER"] = target_provider
    ...
    async def _handle_provider_command(self, event: MessageEvent) -> str:
        ...
        lines.append("Setup: `gauss setup`")
        return "\n".join(lines)
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        ...
        if not personalities:
            return "No personalities configured in `~/.gauss/config.yaml`"
        ...
        try:
            ...
            if "pynacl" in err_lower or "nacl" in err_lower or "davey" in err_lower:
                return (
                    ...
                    "Install or reinstall Gauss with the messaging extra, e.g. "
                    "`pip install gauss-agent[messaging]`."
                )
    ...
    async def _run_background_task(
        ...
        try:
            ...
            default_toolset_map = {
                Platform.LOCAL: "gauss-cli",
                Platform.TELEGRAM: "gauss-telegram",
                Platform.DISCORD: "gauss-discord",
                Platform.WHATSAPP: "gauss-whatsapp",
                Platform.SLACK: "gauss-slack",
                Platform.SIGNAL: "gauss-signal",
                Platform.HOMEASSISTANT: "gauss-homeassistant",
                Platform.EMAIL: "gauss-email",
            }
            ...
            if config_toolsets and isinstance(config_toolsets, list):
                ...
                default_toolset = default_toolset_map.get(source.platform, "gauss-telegram")
                enabled_toolsets = [default_toolset]

            platform_key = "cli" if source.platform == Platform.LOCAL else source.platform.value

            pr = self._provider_routing
            max_iterations = int(os.getenv("GAUSS_MAX_ITERATIONS", "90"))
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        """Handle /update command — update Gauss Agent to the latest version.

        Spawns ``gauss update`` in a separate systemd scope so it survives the
        gateway restart that ``gauss update`` may trigger at the end. Marker
        ...
        gauss_cmd = _resolve_gauss_bin()
        if not gauss_cmd:
            return (
                "✗ Could not locate the `gauss` command. "
                "Gauss is running, but the update command could not find the "
                "executable on PATH or via the current Python interpreter. "
                "Try running `gauss update` manually in your terminal."
            )

        pending_path = _gauss_home / ".update_pending.json"
        output_path = _gauss_home / ".update_output.txt"
        exit_code_path = _gauss_home / ".update_exit_code"
        pending = {
        ...
        # Spawn `gauss update` in a separate cgroup so it survives gateway
        # restart. systemd-run --user --scope creates a transient scope unit.
        gauss_cmd_str = " ".join(shlex.quote(part) for part in gauss_cmd)
        update_cmd = (
            f"{gauss_cmd_str} update > {shlex.quote(str(output_path))} 2>&1; "
        ...
        self._schedule_update_notification_watch()
        return "⚕ Starting Gauss update… I'll notify you when it's done."
    ...
    async def _watch_for_update_completion(
        ...
        """Wait for ``gauss update`` to finish, then send its notification."""
        pending_path = _gauss_home / ".update_pending.json"
        claimed_path = _gauss_home / ".update_pending.claimed.json"
        exit_code_path = _gauss_home / ".update_exit_code"
            ...
            if adapter and chat_id:
                ...
                if output:
                    if len(output) > 3500:
                        output = "…" + output[-3500:]
                    if exit_code == 0:
                        msg = f"✅ Gauss update finished.\n\n```\n{output}\n```"
                    else:
                        msg = f"❌ Gauss update failed.\n\n```\n{output}\n```"
                else:
                    if exit_code == 0:
                        msg = "✅ Gauss update finished successfully."
                    else:
                        msg = "❌ Gauss update failed. Check the gateway logs or run `gauss update` manually for details."
    ...
    def _set_session_env(self, context: SessionContext) -> None:
        ...
        os.environ["GAUSS_SESSION_PLATFORM"] = context.source.platform.value
        os.environ["GAUSS_SESSION_CHAT_ID"] = context.source.chat_id
        if context.source.chat_name:
            os.environ["GAUSS_SESSION_CHAT_NAME"] = context.source.chat_name
        if context.source.thread_id:
            os.environ["GAUSS_SESSION_THREAD_ID"] = str(context.source.thread_id)
    
    def _clear_session_env(self) -> None:
        """Clear session environment variables."""
        for var in ["GAUSS_SESSION_PLATFORM", "GAUSS_SESSION_CHAT_ID", "GAUSS_SESSION_CHAT_NAME", "GAUSS_SESSION_THREAD_ID"]:
    ...
    async def _run_agent(
        ...
        _progress_cfg = {}
        try:
            _tp_cfg_path = _gauss_home / "config.yaml"
        ...
        progress_mode = (
            ...
            or os.getenv("GAUSS_TOOL_PROGRESS_MODE")
            or "all"
        )
        ...
        def run_sync():
            ...
            os.environ["GAUSS_SESSION_KEY"] = session_key or ""
...
def _start_cron_ticker(stop_event: threading.Event, adapters=None, interval: int = 60):
    ...
    needing a separate `gauss cron daemon` or system cron entry.
...
async def start_gateway(config: Optional[GatewayConfig] = None, replace: bool = False) -> bool:
    ...
    # Prevent two gateways from running under the same GAUSS_HOME.
    # The PID file is scoped to GAUSS_HOME, so future multi-profile
    # setups (each profile using a distinct GAUSS_HOME) will naturally
    ...
    if existing_pid is not None and existing_pid != os.getpid():
        if replace:
            ...
            gauss_home = os.getenv("GAUSS_HOME", "~/.gauss")
            logger.error(
                "Another gateway instance is already running (PID %d, GAUSS_HOME=%s). "
                "Use 'gauss gateway restart' to replace it, or 'gauss gateway stop' first.",
                existing_pid, gauss_home,
            )
            print(
                f"\n❌ Gateway already running (PID {existing_pid}).\n"
                f"   Use 'gauss gateway restart' to replace it,\n"
                f"   or 'gauss gateway stop' to kill it first.\n"
                f"   Or use 'gauss gateway run --replace' to auto-replace.\n"
            )
            return False

    # Configure rotating file log so gateway output is persisted for debugging
    log_dir = _gauss_home / 'logs'
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Gauss Gateway - Multi-platform messaging")
