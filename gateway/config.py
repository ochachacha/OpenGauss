...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_gauss_home() / "sessions")
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        sessions_dir = get_gauss_home() / "sessions"
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.gauss/gateway.json
    3. cli-config.yaml gateway section
    4. Defaults
    """
    config = GatewayConfig()
    
    # Try loading from ~/.gauss/gateway.json
    _home = get_gauss_home()
    ...
    # for session reset policy since that's where gauss setup writes it.
    try:
        import yaml
...
def save_gateway_config(config: GatewayConfig) -> None:
    """Save gateway configuration to ~/.gauss/gateway.json."""
    gateway_config_path = get_gauss_home() / "gateway.json"
