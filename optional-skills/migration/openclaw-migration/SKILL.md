...
description: Migrate a user's OpenClaw customization footprint into Gauss Agent. Imports Gauss-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.
version: 1.0.0
author: Gauss Agent (Nous Research)
license: MIT
metadata:
  gauss:
    tags: [Migration, OpenClaw, Gauss, Memory, Persona, Import]
    related_skills: [gauss-agent]
---

# OpenClaw -> Gauss Migration

Use this skill when a user wants to move their OpenClaw setup into Gauss Agent with minimal manual cleanup.
...
```bash
gauss claw migrate              # Full interactive migration
gauss claw migrate --dry-run    # Preview what would be migrated
gauss claw migrate --preset user-data   # Migrate without secrets
gauss claw migrate --overwrite  # Overwrite existing conflicts
gauss claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `gauss setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_gauss.py` to:

- import `SOUL.md` into the Gauss home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Gauss memory entries
- merge OpenClaw command approval patterns into Gauss `command_allowlist`
- migrate Gauss-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS` and `MESSAGING_CWD`
- copy OpenClaw skills into `~/.gauss/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Gauss workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.gauss/tts/`
- archive non-secret docs that do not have a direct Gauss destination
...
- `scripts/openclaw_to_gauss.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.gauss/skills/migration/openclaw-migration/scripts/openclaw_to_gauss.py`

Do not guess a shorter path like `~/.gauss/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.gauss/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Gauss-compatible secrets, currently:
...
Gauss CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Gauss. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Gauss workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Gauss skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.gauss/skills/migration/openclaw-migration/scripts/openclaw_to_gauss.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.gauss/skills/migration/openclaw-migration/scripts/openclaw_to_gauss.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.gauss/skills/migration/openclaw-migration/scripts/openclaw_to_gauss.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.gauss/skills/migration/openclaw-migration/scripts/openclaw_to_gauss.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.gauss/skills/migration/openclaw-migration/scripts/openclaw_to_gauss.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.gauss/skills/migration/openclaw-migration/scripts/openclaw_to_gauss.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Gauss unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Gauss targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Gauss destination. Unsupported auth blobs must still be reported as skipped.
...
- Gauss persona state imported
- Gauss memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.gauss/skills/openclaw-imports/`
