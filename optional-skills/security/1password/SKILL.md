...
- `tmux` available for stable authenticated sessions during Gauss terminal calls (desktop app flow only)
...
### Service Account (recommended for Gauss)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.gauss/.env` (the skill will prompt for this on first load).
...
## Gauss Execution Pattern (desktop app flow)
