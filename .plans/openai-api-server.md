# OpenAI-Compatible API Server for Gauss Agent
...
SSE streaming. By exposing this endpoint, gauss-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  gauss-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.gauss/.env`
2. Run `gauss gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with gauss-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns gauss-agent as a model) |
...
Authorization: Bearer gauss-api-key-here
...
But gauss-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "gauss-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured gauss-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of gauss-agent's core prompt
...
Once implemented, gauss-agent works as a drop-in backend for:
