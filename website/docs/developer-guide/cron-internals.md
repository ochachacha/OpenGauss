...
description: "How Gauss stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---

# Cron Internals

Gauss cron support is implemented primarily in:
...
Cron jobs are stored in Gauss-managed local state (`~/.gauss/cron/jobs.json`) with atomic write semantics.
...
A cron job may attach multiple skills. At runtime, Gauss loads those skills in order and then appends the job prompt as the task instruction.
...
Gauss uses lock-based protections so overlapping scheduler ticks do not execute the same due-job batch twice.
