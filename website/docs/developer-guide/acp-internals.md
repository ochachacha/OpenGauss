...
The ACP adapter wraps Gauss' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
gauss acp / gauss-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> load ~/.gauss/.env
  -> configure stderr logging
  -> construct GaussACPAgent
  -> acp.run_agent(agent)
```
...
- `allow_once` -> Gauss `once`
- `allow_always` -> Gauss `always`
- reject options -> Gauss `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Gauss tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["gauss-acp"])
...
Instead it reuses Gauss' runtime resolver:

- `acp_adapter/auth.py`
- `gauss_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Gauss provider/credentials.
...
- `toolsets.py` — `gauss-acp` toolset definition
- `gauss_cli/main.py` — `gauss acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `gauss-acp` script
