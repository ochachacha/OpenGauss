...
Gauss currently supports three API execution modes:
...
Gauss wraps API requests so they can be interrupted from the CLI or gateway.
...
Gauss uses two execution strategies:
...
Gauss tracks a shared iteration budget across parent and subagents. It also injects budget pressure hints near the end of the available iteration window.
...
Before and during long runs, Gauss may:
