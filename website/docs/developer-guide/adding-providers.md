...
description: "How to add a new inference provider to Gauss Agent — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---

# Adding Providers

Gauss can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:

- provider-specific auth or token refresh
- a curated model catalog
- setup / `gauss model` menu entries
...
1. `gauss_cli/auth.py` decides how credentials are found.
2. `gauss_cli/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `gauss_cli/models.py`, `gauss_cli/main.py`, and `gauss_cli/setup.py` make the provider show up in the CLI.
...
1. `gauss_cli/auth.py`
2. `gauss_cli/models.py`
3. `gauss_cli/runtime_provider.py`
4. `gauss_cli/main.py`
5. `gauss_cli/setup.py`
...
- `PROVIDER_REGISTRY` in `gauss_cli/auth.py`
- `_PROVIDER_LABELS` in `gauss_cli/models.py`
- `_PROVIDER_ALIASES` in both `gauss_cli/auth.py` and `gauss_cli/models.py`
- CLI `--provider` choices in `gauss_cli/main.py`
...
## Step 2: Add auth metadata in `gauss_cli/auth.py`
...
- What env vars should Gauss check, and in what priority order?
...
## Step 3: Add model catalog and aliases in `gauss_cli/models.py`
...
## Step 4: Resolve runtime data in `gauss_cli/runtime_provider.py`
...
Be careful with API-key precedence. Gauss already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.

## Step 5: Wire the CLI in `gauss_cli/main.py` and `gauss_cli/setup.py`
...
If you only update one of these files, `gauss model` and `gauss setup` will drift.
...
When you add a native provider, double-check that Gauss is only sending fields that provider actually understands.
...
```bash
source .venv/bin/activate
python -m gauss_cli.main chat -q "Say hello" --provider your-provider --model your-model
```
...
- [ ] `ProviderConfig` added in `gauss_cli/auth.py`
- [ ] aliases added in `gauss_cli/auth.py` and `gauss_cli/models.py`
- [ ] model catalog added in `gauss_cli/models.py`
- [ ] runtime branch added in `gauss_cli/runtime_provider.py`
- [ ] CLI wiring added in `gauss_cli/main.py`
- [ ] setup wiring added in `gauss_cli/setup.py`
...
### 7. Updating `gauss model` but not `gauss setup`
