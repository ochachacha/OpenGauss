...
description: "Building RL training environments, running evaluation benchmarks, and generating SFT data with the Gauss-Agent Atropos integration"
---

# Environments, Benchmarks & Data Generation

Gauss Agent includes a full environment framework that connects its tool-calling capabilities to the [Atropos](https://github.com/NousResearch/atropos) RL training framework. This enables three workflows:
...
The Python environment framework documented here lives under the repo's `environments/` directory and is the implementation-level API for Gauss/Atropos integration. This is separate from the user-facing `rl_*` tools, which operate as an orchestration surface for remote RL training workflows.
:::
...
classDiagram
    ...
    BaseEnv <|-- GaussAgentBaseEnv
    GaussAgentBaseEnv <|-- TerminalTestEnv
    GaussAgentBaseEnv <|-- GaussSweEnv
    GaussAgentBaseEnv <|-- TerminalBench2EvalEnv
...
The gauss-agent layer (`environments/gauss_base_env.py`). Adds:
- **Terminal backend configuration** — sets `TERMINAL_ENV` for sandboxed execution (local, Docker, Modal, Daytona, SSH, Singularity)
- **Tool resolution** — `_resolve_tools_for_group()` calls gauss-agent's `get_tool_definitions()` to get the right tool schemas based on enabled/disabled toolsets
- **Agent loop integration** — `collect_trajectory()` runs `GaussAgentLoop` and scores the result
...
Your environment inherits from `GaussAgentBaseEnv` and implements five methods:
...
`GaussAgentLoop` (`environments/agent_loop.py`) is the reusable multi-turn agent engine. It runs the same tool-calling pattern as gauss-agent's main loop:
...
| **Generic** | `call_tool(name, args)` — escape hatch for any gauss-agent tool |
...
parser = get_parser("gauss")  # or "mistral", "llama3_json", "qwen", "deepseek_v3", etc.
content, tool_calls = parser.parse(raw_model_output)
```

Available parsers: `gauss`, `mistral`, `llama3_json`, `qwen`, `qwen3_coder`, `deepseek_v3`, `deepseek_v3_1`, `kimi_k2`, `longcat`, `glm45`, `glm47`.
...
pip install "gauss-agent[yc-bench]"
...
```bash
python environments/gauss_swe_env/gauss_swe_env.py serve \
...
- Set `tool_call_parser` in config to match your model's format (e.g., `"gauss"`, `"qwen"`, `"mistral"`)
...
class MyEnvConfig(GaussAgentEnvConfig):
    my_custom_field: str = "default_value"

class MyEnv(GaussAgentBaseEnv):
    name = "my-env"
...
### GaussAgentEnvConfig Fields

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enabled_toolsets` | `List[str]` | `None` (all) | Which gauss toolsets to enable |
...
| `tool_call_parser` | `str` | `"gauss"` | Parser for Phase 2 raw output |
...
env:
  ...
  tokenizer_name: "NousResearch/Gauss-3-Llama-3.1-8B"
  use_wandb: true
...
- [Modal](https://modal.com) account and CLI: `pip install "gauss-agent[modal]"`
- `MODAL_TOKEN_ID` and `MODAL_TOKEN_SECRET` environment variables

### For YC-Bench

- `pip install "gauss-agent[yc-bench]"` (installs the yc-bench CLI + SQLAlchemy)
...
```
environments/
├── gauss_base_env.py          # Abstract base class (GaussAgentBaseEnv)
├── agent_loop.py               # Multi-turn agent engine (GaussAgentLoop)
├── tool_context.py             # Per-rollout tool access for reward functions
├── patches.py                  # Async-safety patches for Modal backend
│
├── tool_call_parsers/          # Phase 2 client-side parsers
│   ├── gauss_parser.py        # Gauss/ChatML <tool_call> format
...
├── gauss_swe_env/             # SWE-bench training environment
│
