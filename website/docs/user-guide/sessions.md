...
Gauss Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.gauss/state.db`) — structured session metadata with FTS5 full-text search
2. **JSONL transcripts** (`~/.gauss/sessions/`) — raw conversation transcripts including tool calls (gateway)
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`gauss` or `gauss chat`) |
...
gauss -c "my project"   # → resumes "my project #3"
```
...
gauss --resume 20250305_091523_a1b2c3d4
gauss -r 20250305_091523_a1b2c3d4

# Resume by title
gauss --resume "refactoring auth"

# Or with the chat subcommand
gauss chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `gauss sessions list`.

### Conversation Recap on Resume

When you resume a session, Gauss displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src="/img/docs/session-recap.svg" alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Gauss session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.gauss/config.yaml`:
...
```bash
gauss sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Gauss creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`gauss -c "my project"`), it automatically picks the most recent session in the lineage.
...
Gauss provides a full set of session management commands via `gauss sessions`:
...
# Filter by platform
gauss sessions list --source telegram

# Show more sessions
gauss sessions list --limit 50
```
...
gauss sessions export backup.jsonl

# Export sessions from a specific platform
gauss sessions export telegram-history.jsonl --source telegram

# Export a single session
gauss sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
gauss sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
gauss sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
gauss sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
gauss sessions prune

# Custom age threshold
gauss sessions prune --older-than 30

# Only prune sessions from a specific platform
gauss sessions prune --source telegram --older-than 60

# Skip confirmation
gauss sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`gauss insights`](/docs/reference/cli-commands#gauss-insights).
...
When Gauss cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Gauss uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Gauss in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.gauss/state.db` | All session metadata + messages with FTS5 |
| Gateway transcripts | `~/.gauss/sessions/` | JSONL transcripts per session + sessions.json index |
| Gateway index | `~/.gauss/sessions/sessions.json` | Maps session keys to active session IDs |
...
gauss sessions delete <session_id>
