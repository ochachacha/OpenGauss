...
Each hook is a directory under `~/.gauss/hooks/` containing two files:
...
LOG_FILE = Path.home() / ".gauss" / "hooks" / "my-hook" / "activity.log"
...
```yaml
# ~/.gauss/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.gauss/hooks/long-task-alert/handler.py
import os
import httpx
...
```yaml
# ~/.gauss/hooks/command-logger/HOOK.yaml
name: command-logger
...
```python
# ~/.gauss/hooks/command-logger/handler.py
import json
from datetime import datetime
from pathlib import Path

LOG = Path.home() / ".gauss" / "logs" / "command_usage.jsonl"
...
```yaml
# ~/.gauss/hooks/session-webhook/HOOK.yaml
...
```python
# ~/.gauss/hooks/session-webhook/handler.py
import httpx

WEBHOOK_URL = "https://your-service.example.com/gauss-events"
...
1. On gateway startup, `HookRegistry.discover_and_load()` scans `~/.gauss/hooks/`
