...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Gauss Agent — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Gauss Agent supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Gauss](/docs/guides/use-voice-mode-with-gauss).

## Prerequisites

Before using voice features, make sure you have:

1. **Gauss Agent installed** — `pip install gauss-agent` (see [Installation](/docs/getting-started/installation))
2. **An LLM provider configured** — run `gauss model` or set your preferred provider credentials in `~/.gauss/.env`
3. **A working base setup** — run `gauss` to verify the agent responds to text before enabling voice

:::tip
The `~/.gauss/` directory and default `config.yaml` are created automatically the first time you run `gauss`. You only need to create `~/.gauss/.env` manually for API keys.
:::
...
pip install gauss-agent[voice]

# Discord + Telegram messaging (includes discord.py[voice] for VC support)
pip install gauss-agent[messaging]

# Premium TTS (ElevenLabs)
pip install gauss-agent[tts-premium]
...
1. Start the CLI with `gauss` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.gauss/config.yaml` (default: `ctrl+b`).
:::
...
```bash
gauss gateway        # Start the gateway (connects to configured platforms)
gauss gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@gaussbyt4 hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.gauss/.env`:
```bash
...
```bash
gauss gateway        # Start with existing configuration
```
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.gauss/.env`
...
- Review gateway logs: `tail -f ~/.gauss/logs/gateway.log`
