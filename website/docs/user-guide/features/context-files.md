...
Gauss Agent automatically discovers and loads context files that shape how it behaves. Some are project-local and discovered from your working directory. `SOUL.md` is now global to the Gauss instance and is loaded from `GAUSS_HOME` only.
...
| **SOUL.md** | Global personality and tone customization for this Gauss instance | `GAUSS_HOME/SOUL.md` only |
...
Gauss walks the directory tree starting from the working directory and loads **all** `AGENTS.md` files found, sorted by depth. This supports monorepo-style setups:
...
- `~/.gauss/SOUL.md`
- or `$GAUSS_HOME/SOUL.md` if you run Gauss with a custom home directory

Important details:

- Gauss seeds a default `SOUL.md` automatically if one does not exist yet
- Gauss loads `SOUL.md` only from `GAUSS_HOME`
- Gauss does not probe the working directory for `SOUL.md`
...
Gauss is compatible with Cursor IDE's `.cursorrules` file and `.cursor/rules/*.mdc` rule modules. If these files exist in your project root, they're loaded alongside AGENTS.md.

This means your existing Cursor conventions automatically apply when using Gauss.
