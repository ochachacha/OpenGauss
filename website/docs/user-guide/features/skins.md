...
sidebar_position: 10
title: "Skins & Themes"
description: "Customize the Gauss CLI with built-in and user-defined skins"
---

# Skins & Themes

Skins control the **visual presentation** of the Gauss CLI: banner colors, spinner faces and verbs, response-box labels, branding text, and the tool activity prefix.
...
/skin mytheme        # switch to a custom skin from ~/.gauss/skins/mytheme.yaml
```

Or set the default skin in `~/.gauss/config.yaml`:
...
|------|-------------|----------------|
| `default` | Classic Gauss — gold and kawaii | `Gauss Agent` |
| `ares` | War-god theme — crimson and bronze | `Ares Agent` |
| `mono` | Monochrome — clean grayscale | `Gauss Agent` |
| `slate` | Cool blue — developer-focused | `Gauss Agent` |
...
Create YAML files under `~/.gauss/skins/`. User skins inherit missing values from the built-in `default` skin.
...
- Built-in skins load from `gauss_cli/skin_engine.py`.
