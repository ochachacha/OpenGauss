...
All skills live in **`~/.gauss/skills/`** — a single directory that serves as the source of truth. On fresh install, bundled skills are copied from the repo. Hub-installed and agent-created skills also go here. The agent can modify or delete any skill.
...
The bundled `plan` skill is a good example of a skill-backed slash command with custom behavior. Running `/plan [request]` tells Gauss to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.gauss/plans/` relative to the active workspace/backend working directory.

You can also interact with skills through natural conversation:

```bash
gauss chat --toolsets skills -q "What skills do you have?"
gauss chat --toolsets skills -q "Show me the axolotl skill"
```
...
When a missing value is encountered, Gauss asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `gauss setup` or `~/.gauss/.env` locally instead.

## Skill Directory Structure

```text
~/.gauss/skills/                  # Single source of truth
...
```bash
gauss skills browse                              # Browse all hub skills (official first)
gauss skills browse --source official            # Browse only official optional skills
gauss skills search kubernetes                   # Search all sources
gauss skills search react --source skills-sh     # Search the skills.sh directory
gauss skills search https://mintlify.com/docs --source well-known
gauss skills inspect openai/skills/k8s           # Preview before installing
gauss skills install openai/skills/k8s           # Install with security scan
gauss skills install official/security/1password
gauss skills install skills-sh/vercel-labs/json-render/json-render-react --force
gauss skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
gauss skills list --source hub                   # List hub-installed skills
gauss skills check                               # Check installed hub skills for upstream updates
gauss skills update                              # Reinstall hub skills with upstream changes when needed
gauss skills audit                               # Re-scan all hub skills for security
gauss skills uninstall k8s                       # Remove a hub skill
gauss skills publish skills/my-skill --to github --repo owner/repo
gauss skills snapshot export setup.json          # Export skill config
gauss skills tap add myorg/skills-repo           # Add a custom GitHub source
```
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with Gauss. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `gauss skills search <query> --source skills-sh`. Gauss resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
Gauss currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the Gauss repository itself and install with builtin trust.
...
This is Vercel's public skills directory. Gauss can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
```bash
gauss skills search https://mintlify.com/docs --source well-known
gauss skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
gauss skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```

#### 4. Direct GitHub skills (`github`)

Gauss can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
```bash
gauss skills install openai/skills/k8s
gauss skills tap add myorg/skills-repo
```
...
- Gauss source id: `clawhub`

#### 6. Claude marketplace-style repos (`claude-marketplace`)

Gauss supports marketplace repos that publish Claude-compatible plugin/marketplace manifests.

Known integrated sources include:
- [anthropics/skills](https://github.com/anthropics/skills)
- [aiskillstore/marketplace](https://github.com/aiskillstore/marketplace)

Gauss source id: `claude-marketplace`

#### 7. LobeHub (`lobehub`)

Gauss can search and convert agent entries from LobeHub's public catalog into installable Gauss skills.

- Site: [LobeHub](https://lobehub.com/)
- Public agents index: [chat-agents.lobehub.com](https://chat-agents.lobehub.com/)
- Backing repo: [lobehub/lobe-chat-agents](https://github.com/lobehub/lobe-chat-agents)
- Gauss source id: `lobehub`
...
`gauss skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
| Level | Source | Policy |
|-------|--------|--------|
| `builtin` | Ships with Gauss | Always trusted |
...
```bash
gauss skills check          # Report which installed hub skills changed upstream
gauss skills update         # Reinstall only the skills with updates available
gauss skills update react   # Update one specific installed hub skill
```
