...
description: "Customize Gauss Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Gauss Agent's personality is customizable, but there are two different layers that matter:

- `SOUL.md` — a durable persona file that lives in `GAUSS_HOME` and is loaded automatically for that Gauss instance
...
Gauss now seeds a default `SOUL.md` automatically in:

```text
~/.gauss/SOUL.md
```

More precisely, it uses the current instance's `GAUSS_HOME`, so if you run Gauss with a custom home directory, it will use:
...
- Gauss creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Gauss loads `SOUL.md` only from `GAUSS_HOME`
- Gauss does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, Gauss adds nothing from it to the prompt
- If `SOUL.md` has content, that content is injected verbatim after security scanning and truncation
- Gauss does not add wrapper language like "If SOUL.md is present..." around the file anymore
...
If Gauss loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `GAUSS_HOME`, the personality belongs to the Gauss instance itself.

That also makes it easier to teach users:
- "Edit `~/.gauss/SOUL.md` to change Gauss' default personality."
...
- how Gauss should handle uncertainty, disagreement, or ambiguity
...
## What Gauss injects into the prompt

If `SOUL.md` contains text, Gauss injects the file's text itself — not a wrapper explanation.
...
Gauss ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Gauss, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Gauss its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.gauss/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.gauss/SOUL.md`
...
- [SOUL.md Guide](/docs/guides/use-soul-with-gauss)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Gauss speaks
- `display.skin` and `/skin` affect how Gauss looks in the terminal
