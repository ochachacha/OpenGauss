...
description: "Connect Gauss Agent to external tool servers via MCP — and control exactly which MCP tools Gauss loads"
---

# MCP (Model Context Protocol)

MCP lets Gauss Agent connect to external tool servers so the agent can use tools that live outside Gauss itself — GitHub, databases, file systems, browser stacks, internal APIs, and more.

If you have ever wanted Gauss to use a tool that already exists somewhere else, MCP is usually the cleanest way to do it.

## What MCP gives you

- Access to external tool ecosystems without writing a native Gauss tool first
- Local stdio servers and remote HTTP MCP servers in the same config
- Automatic tool discovery and registration at startup
- Utility wrappers for MCP resources and prompts when supported by the server
- Per-server filtering so you can expose only the MCP tools you actually want Gauss to see
...
2. Add an MCP server to `~/.gauss/config.yaml`:
...
4. Ask Gauss to use the MCP-backed capability.
...
Gauss will discover the MCP server's tools and use them like any other tool.
...
HTTP MCP servers are remote endpoints Gauss connects to directly.
...
- you do not want Gauss spawning a local subprocess for that integration

## Basic configuration reference

Gauss reads MCP config from `~/.gauss/config.yaml` under `mcp_servers`.
...
| `enabled` | bool | If `false`, Gauss skips the server entirely |
...
## How Gauss registers MCP tools

Gauss prefixes MCP tools so they do not collide with built-in names:
...
In practice, you usually do not need to call the prefixed name manually — Gauss sees the tool and chooses it during normal reasoning.

## MCP utility tools

When supported, Gauss also registers utility tools around MCP resources and prompts:
...
- Gauss only registers resource utilities if the MCP session actually supports resource operations
- Gauss only registers prompt utilities if the MCP session actually supports prompt operations
...
You can now control which tools each MCP server contributes to Gauss.
...
If `enabled: false`, Gauss skips the server completely and does not even attempt a connection.
...
You can also separately disable Gauss-added utility wrappers:
...
If your config filters out all callable tools and disables or omits all supported utilities, Gauss does not create an empty runtime MCP toolset for that server.
...
Gauss discovers MCP servers at startup and registers their tools into the normal tool registry.
...
For stdio servers, Gauss does not blindly pass your full shell environment.
...
Then verify your config and restart Gauss.
...
Because Gauss now only registers those wrappers when both are true:
