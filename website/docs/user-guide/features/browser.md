...
Gauss Agent includes a full browser automation toolset that can run in two modes:
...
If you do **not** set Browserbase credentials, Gauss can still use the browser tools through a local Chromium install driven by `agent-browser`.
...
:::info
The `browser` toolset must be included in your config's `toolsets` list or enabled via `gauss config set toolsets '["gauss-cli", "browser"]'`.
:::
...
```
Type "gauss agent" into the search field @e3
```
...
Screenshots are stored in `~/.gauss/browser_screenshots/` and automatically cleaned up after 24 hours.
...
When enabled, recording starts automatically on the first `browser_navigate` and saves to `~/.gauss/browser_recordings/` when the session closes. Works in both local and cloud (Browserbase) modes. Recordings older than 72 hours are automatically cleaned up.
...
:::note
If paid features aren't available on your plan, Gauss automatically falls back — first disabling `keepAlive`, then proxies — so browsing still works on free plans.
:::
