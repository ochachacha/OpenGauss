...
Schedule tasks to run automatically with natural language or cron expressions. Gauss exposes cron management through a single `cronjob` tool with action-style operations instead of separate schedule/list/remove tools.
...
:::warning
Cron-run sessions cannot recursively create more cron jobs. Gauss disables cron management tools inside cron executions to prevent runaway scheduling loops.
:::
...
```bash
gauss cron create "every 2h" "Check server status"
gauss cron create "every 1h" "Summarize new feed items" --skill blogwatcher
gauss cron create "every 1h" "Use both skills and combine the result" \
  --skill blogwatcher \
  --skill find-nearby \
  --name "Skill combo"
```
...
Gauss will use the unified `cronjob` tool internally.
...
```bash
gauss cron edit <job_id> --schedule "every 4h"
gauss cron edit <job_id> --prompt "Use the revised task"
gauss cron edit <job_id> --skill blogwatcher --skill find-nearby
gauss cron edit <job_id> --add-skill find-nearby
gauss cron edit <job_id> --remove-skill blogwatcher
gauss cron edit <job_id> --clear-skills
```
...
```bash
gauss cron list
gauss cron pause <job_id>
gauss cron resume <job_id>
gauss cron run <job_id>
gauss cron remove <job_id>
gauss cron status
gauss cron tick
```
...
```bash
gauss gateway install     # Install as a user service
sudo gauss gateway install --system   # Linux: boot-time system service for servers
gauss gateway             # Or run in foreground
...
1. loads jobs from `~/.gauss/cron/jobs.json`
...
A file lock at `~/.gauss/cron/.tick.lock` prevents overlapping scheduler ticks from double-running the same job batch.
...
| `"local"` | Save to local files only (`~/.gauss/cron/output/`) | Default on CLI |
...
The agent's final response is automatically delivered — you do **not** need to include `send_message` in the cron prompt for that same destination. If a cron run calls `send_message` to the exact target the scheduler will already deliver to, Gauss skips that duplicate send and tells the model to put the user-facing content in the final response instead. Use `send_message` only for additional or different targets.
...
Jobs are stored in `~/.gauss/cron/jobs.json`. Output from job runs is saved to `~/.gauss/cron/output/{job_id}/{timestamp}.md`.
