...
sidebar_position: 9
title: "Git Worktrees"
description: "Run multiple Gauss agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Gauss Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Gauss so each session has a clean, isolated working directory.

## Why Use Worktrees with Gauss?

Gauss treats the **current working directory** as the project root:

- CLI: the directory where you run `gauss` or `gauss chat`
...
git worktree add ../repo-feature feature/gauss-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/gauss-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Gauss there:

```bash
cd ../repo-feature

# Start Gauss in the worktree
gauss
```
...
git worktree add ../repo-experiment-a feature/gauss-a
git worktree add ../repo-experiment-b feature/gauss-b
```
...
- Works on its own branch (`feature/gauss-a` vs `feature/gauss-b`).
...
- Gauss checkpoint data under `~/.gauss/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Gauss experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/gauss-checkpoints-docs`, `feature/gauss-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Gauss from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `gauss -w` (Automatic Worktree Mode)

Gauss has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `gauss/gauss-<hash>`).
...
```bash
gauss -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `gauss -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Gauss session its own clean checkout.
