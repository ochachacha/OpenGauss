...
# Build a Gauss Plugin

This guide walks through building a complete Gauss plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill — everything the plugin system supports.
...
```bash
mkdir -p ~/.gauss/plugins/calculator
cd ~/.gauss/plugins/calculator
```
...
This tells Gauss: "I'm a plugin called calculator, I provide tools and hooks." That's all the manifest needs.
...
4. **Accept `**kwargs`:** Gauss may pass additional context in the future.
...
- If this function crashes, the plugin is disabled but Gauss continues fine
...
def _install_skill():
    """Copy our skill to ~/.gauss/skills/ on first load."""
    try:
        from gauss_cli.config import get_gauss_home
        dest = get_gauss_home() / "skills" / "my-plugin" / "SKILL.md"
    except Exception:
        dest = Path.home() / ".gauss" / "skills" / "my-plugin" / "SKILL.md"
...
```toml
# pyproject.toml
[project.entry-points."gauss_agent.plugins"]
my-plugin = "my_plugin_package"
```

```bash
pip install gauss-plugin-calculator
# Plugin auto-discovered on next gauss startup
```
...
```python
# Wrong — will break if Gauss passes extra context
def handler(args):
    ...
