---
sidebar_position: 5
title: "Use MCP with Gauss"
description: "A practical guide to connecting MCP servers to Gauss Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Gauss

This guide shows how to actually use MCP with Gauss Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Gauss tool
- you want Gauss to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Gauss to internal APIs, databases, or company systems without modifying Gauss core

Do not use MCP when:
- a built-in Gauss tool already solves the job well
...
- Gauss remains the agent
- MCP servers contribute tools
- Gauss discovers those tools at startup or reload time
...
- Gauss banner/status should show MCP integration when configured
- ask Gauss what tools it has available
...
There are two categories of MCP-exposed functionality in Gauss:
...
2. Gauss-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Gauss will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Gauss to reason over a bounded workspace.
...
Now Gauss can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Gauss core.
...
Because Gauss now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
