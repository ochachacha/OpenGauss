---
sidebar_position: 6
title: "Use SOUL.md with Gauss"
description: "How to use SOUL.md to shape Gauss Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Gauss

`SOUL.md` is the easiest way to give Gauss a stable, default voice.

If you want Gauss to feel like the same assistant every time you talk to it — without repeating instructions in every session — this is the file to use.
...
- personality
- communication style
- how direct or warm Gauss should be
- what Gauss should avoid stylistically
- how Gauss should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Gauss is and how Gauss speaks
...
Gauss now uses only the global SOUL file for the current instance:

```text
~/.gauss/SOUL.md
```

If you run Gauss with a custom home directory, it becomes:
...
Gauss automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Gauss does not overwrite it
- if the file exists but is empty, Gauss adds nothing from it to the prompt

## How Gauss uses it

When Gauss starts a session, it reads `SOUL.md` from `GAUSS_HOME`, scans it for prompt-injection patterns, truncates it if needed, and injects the content directly into the prompt.

No wrapper language is added around the file.

So the content itself matters. Write the way you want Gauss to think and speak.
...
That alone can noticeably change how Gauss feels.
...
Gauss already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Gauss should sound.

# Avoid
What Gauss should not do.

# Defaults
How Gauss should behave when ambiguity appears.
```
...
Then restart Gauss or start a new session.
...
### I edited SOUL.md but Gauss still sounds the same

Check:
- you edited `~/.gauss/SOUL.md` or `$GAUSS_HOME/SOUL.md`
...
### Gauss is ignoring parts of my SOUL.md
