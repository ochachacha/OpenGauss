...
This tutorial walks you through setting up a Telegram bot powered by Gauss Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Gauss Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/learning-path) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.gauss/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Gauss itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Gauss Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_gauss_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Gauss Agent. DM me for help with code, research, debugging, and more.
   ```
...
```
[Gateway] Starting Gauss Gateway...
...
sudo gauss gateway install --system   # Linux only: boot-time system service
```
...
```bash
gauss gateway stop && gauss gateway start
```
...
gauss pairing revoke telegram 987654321

# Clear expired pending codes
gauss pairing clear-pending
```
...
**Option 2:** Set it manually in `~/.gauss/.env`:
...
Control how much detail the bot shows when using tools. In `~/.gauss/config.yaml`:
...
Customize how the bot communicates by editing `~/.gauss/SOUL.md`:

For a full guide, see [Use SOUL.md with Gauss](/docs/guides/use-soul-with-gauss).
...
```bash
# From the CLI
gauss cron list          # View all scheduled jobs
gauss cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u gauss-gateway` (Linux) or `~/.gauss/logs/gateway.log` (macOS) |
| Cron job output | `~/.gauss/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.gauss/cron/jobs.json` |
| Pairing data | `~/.gauss/pairing/` |
| Session history | `~/.gauss/sessions/` |
