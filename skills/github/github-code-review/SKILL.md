...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Gauss Agent\",
    \"comments\": [
...
```bash
source ~/.gauss/skills/github/github-auth/scripts/gh-env.sh
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Gauss Agent. Code looks clean — good test coverage, no security concerns."
