...
If you already keep your X credentials in `~/.gauss/.env`, the cleanest setup is:

```bash
mkdir -p ~/.config/x-cli
ln -sf ~/.gauss/.env ~/.config/x-cli/.env
```
...
- **Credential drift**: if you rotate tokens in `~/.gauss/.env`, make sure `~/.config/x-cli/.env` still points at the current file.
