"""
Gateway subcommand for gauss CLI.

Handles: gauss gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
from gauss_cli.config import get_env_value, get_gauss_home, save_env_value
from gauss_cli.setup import (
...
_SERVICE_BASE = "gauss-gateway"
SERVICE_DESCRIPTION = "Gauss Agent Gateway - Messaging Platform Integration"


def get_service_name() -> str:
    """Derive a systemd service name scoped to this GAUSS_HOME.

    Default ``~/.gauss`` returns ``gauss-gateway`` (backward compatible).
    Any other GAUSS_HOME appends a short hash so multiple installations
    ...
    home = _Path(os.getenv("GAUSS_HOME", _Path.home() / ".gauss")).resolve()
    default = (_Path.home() / ".gauss").resolve()
    if home == default:
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    gauss gateway uninstall")
    print_info("    sudo gauss gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:
            print_warning("  System service install requires sudo, so Gauss can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo gauss gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo gauss gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo gauss gateway start --system")
            return scope, False
...
def get_launchd_plist_path() -> Path:
    return Path.home() / "Library" / "LaunchAgents" / "ai.gauss.gateway.plist"
...
def get_gauss_cli_path() -> str:
    """Get the path to the gauss CLI."""
    ...
    return f"{get_python_path()} -m gauss_cli.main"
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    gauss_home = str(Path(os.getenv("GAUSS_HOME", Path.home() / ".gauss")).resolve())
...
User={username}
Group={group_name}
ExecStart={python_path} -m gauss_cli.main gateway run --replace
...
Environment="GAUSS_HOME={gauss_home}"
Restart=on-failure
RestartSec=10
KillMode=mixed
KillSignal=SIGTERM
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Gauss install")
    return True
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}gauss gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}gauss gateway status{scope_flag}             # Check status")
...
def systemd_status(deep: bool = False, system: bool = False):
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}gauss gateway install{scope_flag}")
        return
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}gauss gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}gauss gateway start{scope_flag}")
...
<dict>
    <key>Label</key>
    <string>ai.gauss.gateway</string>
    
    <key>ProgramArguments</key>
    <array>
        <string>{python_path}</string>
        <string>-m</string>
        <string>gauss_cli.main</string>
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  gauss gateway status             # Check status")
    print("  tail -f ~/.gauss/logs/gateway.log  # View logs")
...
def launchd_start():
    subprocess.run(["launchctl", "start", "ai.gauss.gateway"], check=True)
    print("✓ Service started")

def launchd_stop():
    subprocess.run(["launchctl", "stop", "ai.gauss.gateway"], check=True)
...
def launchd_status(deep: bool = False):
    result = subprocess.run(
        ["launchctl", "list", "ai.gauss.gateway"],
        capture_output=True,
        text=True
    )
    ...
    if deep:
        log_file = get_gauss_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Gauss agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address Gauss will use (e.g., gauss@gmail.com)."},
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_gauss_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'gauss pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: gauss pairing approve {platform} {code}")
                else:
                    print_info("  Skipped — configure later with 'gauss gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from gauss_cli.main import cmd_whatsapp
    import argparse
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"GaussAgent\"")
    ...
    if any_configured:
        ...
        if service_running:
            if prompt_yes_no("  Restart the gateway to pick up changes?", True):
                try:
                    if is_linux():
                        ...
                        print_info("Start manually: gauss gateway")
                except subprocess.CalledProcessError as e:
            ...
            if is_linux() or is_macos():
                ...
                print_info("  Run in foreground: gauss gateway")
    else:
        print()
        print_info("No platforms configured. Run 'gauss gateway setup' when ready.")
...
def gateway_command(args):
    ...
    if subcmd == "install":
        ...
        if is_linux():
            ...
            print("Run manually: gauss gateway run")
            sys.exit(1)
        ...
        if is_linux() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                print("  gauss gateway install")
                print("  sudo gauss gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  gauss gateway          # Run in foreground")
                print("  gauss gateway install  # Install as user service")
                print("  sudo gauss gateway install --system  # Install as boot-time system service")
