...
Usage:
    gauss pairing list              # Show all pending + approved users
    gauss pairing approve <platform> <code>  # Approve a pairing code
    gauss pairing revoke <platform> <user_id> # Revoke user access
    gauss pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle gauss pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: gauss pairing {list|approve|revoke|clear-pending}")
        print("Run 'gauss pairing --help' for details.")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(f"  Run 'gauss pairing list' to see pending codes.\n")
