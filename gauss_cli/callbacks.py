...
into prompt_toolkit's event loop. Each function takes the GaussCLI instance
...
from gauss_cli.banner import cprint, _DIM, _RST
from gauss_cli.config import save_env_value_secure
...
def prompt_for_secret(cli, var_name: str, prompt: str, metadata=None) -> dict:
    ...
    The secret is stored in ~/.gauss/.env and never exposed to the model.
    """
    if not getattr(cli, "_app", None):
        ...
        cprint(f"\n{_DIM}  ✓ Stored secret in ~/.gauss/.env as {var_name}{_RST}")
        return {
            **stored,
            "skipped": False,
