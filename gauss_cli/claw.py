"""gauss claw — OpenClaw migration commands.

Usage:
    gauss claw migrate              # Interactive migration from ~/.openclaw
    gauss claw migrate --dry-run    # Preview what would be migrated
    gauss claw migrate --preset full --overwrite  # Full migration, overwrite conflicts
"""
...
from gauss_cli.config import get_gauss_home, get_config_path, load_config, save_config
...
def _find_migration_script() -> Path | None:
    """Find the openclaw_to_gauss.py script in known locations."""
...
def _load_migration_module(script_path: Path):
    ...
    spec = importlib.util.spec_from_file_location("openclaw_to_gauss", script_path)
...
def claw_command(args):
    """Route gauss claw subcommands."""
    action = getattr(args, "claw_action", None)

    if action == "migrate":
        _cmd_migrate(args)
    else:
        print("Usage: gauss claw migrate [options]")
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to Gauss")
        print()
        print("Run 'gauss claw migrate --help' for migration options.")


def _cmd_migrate(args):
    """Run the OpenClaw → Gauss migration."""
    ...
    if not source_dir.is_dir():
        ...
        print_info(f"You can specify a custom path: gauss claw migrate --source /path/to/.openclaw")
        return
    ...
    print_info(f"Target:      {gauss_home}")
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  gauss claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
