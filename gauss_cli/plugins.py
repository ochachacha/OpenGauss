...
1. **User plugins**   – ``~/.gauss/plugins/<name>/``
2. **Project plugins** – ``./.gauss/plugins/<name>/``
3. **Pip plugins**     – packages that expose the ``gauss_agent.plugins``
   entry-point group.
...
ENTRY_POINTS_GROUP = "gauss_agent.plugins"

_NS_PARENT = "gauss_plugins"
...
class PluginManager:
    ...
    def discover_and_load(self) -> None:
        ...
        # 1. User plugins (~/.gauss/plugins/)
        gauss_home = os.environ.get("GAUSS_HOME", os.path.expanduser("~/.gauss"))
        user_dir = Path(gauss_home) / "plugins"
        manifests.extend(self._scan_directory(user_dir, source="user"))

        # 2. Project plugins (./.gauss/plugins/)
        project_dir = Path.cwd() / ".gauss" / "plugins"
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``gauss_plugins.<name>``."""
