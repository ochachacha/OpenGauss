"""
Status command for gauss CLI.

Shows the status of all Gauss Agent components.
"""
...
from gauss_cli.auth import AuthError, resolve_provider
...
def show_status(args):
    """Show status of all Gauss Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)
    cli_name = get_cli_command_name()
    title = f"{get_product_name()} Status"
    
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color(f"│ {title:^55} │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    if not terminal_env:
        ...
        # (gauss status doesn't go through cli.py's config loading)
        try:
            _cfg = load_config()
    ...
    jobs_file = get_gauss_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_gauss_home() / "sessions" / "sessions.json"
