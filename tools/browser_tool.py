...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-gauss_…`` the resulting socket path exceeds the
...
def _create_browserbase_session(task_id: str) -> Dict[str, str]:
    ...
    session_name = f"gauss_{task_id}_{uuid.uuid4().hex[:8]}"
...
def _run_browser_command(
    ...
    try:
        ...
        # Ensure PATH includes Gauss-managed Node first, then standard system dirs.
        gauss_home = Path(os.environ.get("GAUSS_HOME", Path.home() / ".gauss"))
        gauss_node_bin = str(gauss_home / "node" / "bin")

        existing_path = browser_env.get("PATH", "")
        path_parts = [p for p in existing_path.split(":") if p]
        candidate_dirs = [gauss_node_bin] + [p for p in _SANE_PATH.split(":") if p]
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = gauss_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> str:
    ...
    gauss_home = Path(os.environ.get("GAUSS_HOME", Path.home() / ".gauss"))
    screenshots_dir = gauss_home / "browser_screenshots"
