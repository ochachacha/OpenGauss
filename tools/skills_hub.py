#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Gauss Skills Hub.
...
Used by gauss_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            gauss_meta = metadata.get("gauss", {})
            if isinstance(gauss_meta, dict):
                tags = gauss_meta.get("tags", [])
        if not tags:
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                gauss_meta = meta_block.get("gauss", {})
                if isinstance(gauss_meta, dict):
                    tags = gauss_meta.get("tags", [])
