...
Configuration is loaded from ~/.gauss/config.yaml under the 'tts:' key.
...
DEFAULT_OUTPUT_DIR = str(Path(os.getenv("GAUSS_HOME", Path.home() / ".gauss")) / "audio_cache")
MAX_TEXT_LENGTH = 4000


# ===========================================================================
# Config loader -- reads tts: section from ~/.gauss/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.gauss/config.yaml.
    ...
    try:
        ...
        logger.debug("gauss_cli.config not available, using default TTS config")
        return {}
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.gauss/config.yaml (tts: section).
    ...
    platform = os.getenv("GAUSS_SESSION_PLATFORM", "").lower()
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": "Optional custom file path to save the audio. Defaults to ~/.gauss/audio_cache/<timestamp>.mp3"
            }
        },
        "required": ["text"]
    }
