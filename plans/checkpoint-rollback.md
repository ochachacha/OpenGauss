...
5. **User-facing rollback** — `/rollback` slash command (CLI + gateway) to list and restore checkpoints. Also `gauss rollback` CLI subcommand.
...
~/.gauss/checkpoints/
  {sha256(abs_dir)[:16]}/       # Shadow git repo per working directory
    ...
    GAUSS_WORKDIR              # Original dir path (for display)
...
Add to `DEFAULT_CONFIG` in `gauss_cli/config.py`:
...
**CLI subcommand** — `gauss rollback` (optional, lower priority).
...
| `gauss_cli/config.py` | Add `checkpoints` + `checkpoint_max_snapshots` to DEFAULT_CONFIG |
...
3. `gauss_cli/config.py` — config keys
